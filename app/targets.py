...
scantargets = Blueprint("targets", __name__, template_folder="../templates", static_folder="../static")

def format_datetime(dt):
    return dt.strftime("%B %d, %Y, %I:%M %p") 

@scantargets.route('/homedashboard/scantargets/')
def target():
...
@scantargets.route('/homedashboard/scantargets/addtarget', methods=['GET', 'POST'])
def addtarget():
    if "username" not in session:
        return redirect(url_for("app.login"))
...
@scantargets.route('/homedashboard/scantargets/delete/<int:target_id>', methods=['POST'])
